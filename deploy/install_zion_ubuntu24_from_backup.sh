...
# - Installs and tunes MySQL 8 for concurrency
# - Installs Python 3.10.11 with pyenv
# - Creates backend/frontend virtualenvs and installs dependencies
# - Restores database from SQL backup (optional)
# - Creates systemd services for FastAPI and Flet
# - Configures Nginx reverse proxy and HTTPS (certbot)
# - Persists app logs in /tmp/zion_logs
...
# If this SQL file exists, it will be imported as full restore.
# If missing, only create_tables.sql migration is executed.
...
# MySQL tuning defaults (good start for 8GB RAM)
...
require_root() {
  if [[ "${EUID}" -ne 0 ]]; then
    die "Run as root: sudo bash deploy/install_zion_ubuntu24_from_backup.sh"
  fi
}
...
patch_project_configs() {
  log "Patching backend and frontend config.py"
...
restore_or_migrate_database() {
  local migration_file="${PROJECT_DIR}/backend/migrations/create_tables.sql"

  if [[ -f "${DB_RESTORE_FILE}" ]]; then
    log "Restoring database from ${DB_RESTORE_FILE}"
    mysql "${DB_NAME}" < "${DB_RESTORE_FILE}"
  else
    warn "Restore file not found: ${DB_RESTORE_FILE}"
    log "Executing schema migration ${migration_file}"
    mysql "${DB_NAME}" < "${migration_file}"
  fi
}

prepare_log_files() {
  log "Preparing application log files in ${LOG_DIR}"
  mkdir -p "${LOG_DIR}"
...
Logs (exceptions and runtime errors):
